...
export const getPluginInstallPwn = (network: Networkish): PluginInstallItem => {
  const networkName = getNetwork(network).name as SupportedNetwork;
...
export const encodeCreditDelegationAction = async (
  ...
  const hexData = iface.encodeFunctionData('borrowAndTransfer', [
    token,
...
export const encodeMakeOfferAction = (
  ...
  const hexData = iface.encodeFunctionData('makeOffer', [
    collateralCategory,
    collateralAddress,
    collateralId,
    collateralAmount,
...
export const encodeCreateGroupAction = (
  ...
  const hexData = iface.encodeFunctionData('createGroup', [
    groupName,
    addresses,
  ]);
...
export const encodeActionsGroup = (
  ...
  const hexData = iface.encodeFunctionData('registerActions', [
    daoAddress,
    actions,
    0,
  ]);
...
export function votingSettingsToContract(params: VotingSettings) {
  return [
    BigNumber.from(
...
export const encodeApproveAction = (
  ...
  const hexData = iface.encodeFunctionData('approve', [spender, amount]);
...
export const encodeSwapAction = async (
  ...
  const amountToSwap =
    Number(amountIn) * Math.pow(10, tokenInfo?.decimals || 18);
  const iface = Uniswapv3__factory.createInterface();

  const hexData = iface.encodeFunctionData('swap', [
    tokenIn,
    tokenOut,
    fee,
    recipient,
...
export const encodeProvideLiquidityAction = async (
  ...
  const hexData = iface.encodeFunctionData('provideLiquidity', [
    token0,
    token1,
    formatedFeed,
    minPrice,
