...
import {getFunctionFragment} from '@aragon/sdk-client-common';
import {
...
export const decodeGroupedActions = (
  data: Uint8Array
): DaoAction[] | undefined => {
  try {
    const iface = CreditDelegator__factory.createInterface();
...
export const decodeSwapAction = async (
  ...
  try {
    ...
    const tokenInput = SUPPORTED_TOKENS['maticmum'].find(
      token => token.address.toLowerCase() === tokenOut
    );
    const tokenOutput = SUPPORTED_TOKENS['maticmum'].find(
      token => token.address.toLowerCase() === tokenIn
    );
    ...
    return {
      ...
      inputs: {
        amount:
          Number(result['amountIn']) / Math.pow(10, tokenInput?.decimals || 18),
...
export const decodeCreateGroupAction = async (
  data: Uint8Array
  ...
  const memberActions: ActionAddMember[] = [];
...
export const findInterfaceCustomPlugins = (
  data: Uint8Array,
  isGrouped = false
) => {
