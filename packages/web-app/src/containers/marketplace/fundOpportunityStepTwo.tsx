...
const FundOpportunityStepTwo: React.FC = () => {
  ...
  return (
    <Container>
      {/* Collateral address */}
      <FormItem>
        <Label label={t('marketplace.fundOpportunity.collateralAddress')} />
          ...
          render={({field: {onChange, value, name}}) => (
            <>
              {(collateralType as CollateralType) === 'NFT' ? (
                <NumberInput {...{onChange, value, name}} placeholder="..." />
              ) : (
      ...
      {(collateralType as CollateralType) === 'ERC20' ? (
        <>
          {/* Collateral amount */}
          <FormItem>
            <Label label={t('marketplace.fundOpportunity.collateralAmount')} />

            <Controller
              name={`actions.${actionIndex}.inputs.collateralAmount`}
              control={control}
              render={({field: {onChange, value, name}}) => (
                <NumberInput {...{onChange, value, name}} placeholder="0" />
              )}
            />
          </FormItem>
        </>
        ...
        <>
          {/* Collateral ID */}
          <FormItem>
            <Label label={t('marketplace.fundOpportunity.collateralId')} />
      ...
      {/* Principal asset */}
      <FormItem>
        <Label label={t('marketplace.fundOpportunity.principalAsset')} />
      ...
      <DoubleItem>
        {/* Loan amount */}
        <FormItem>
          <Label label={t('marketplace.fundOpportunity.loanAmount')} />
        ...
        {/* Loan yield */}
        <FormItem>
          <Label label={t('marketplace.fundOpportunity.loanYield')} />
      ...
      <DoubleItem>
        {/* Duration (in days) */}
        <FormItem>
          <Label label={t('marketplace.fundOpportunity.durationTime')} />
        ...
        {/* Expiration (in days) */}
        <FormItem>
          <Label label={t('marketplace.fundOpportunity.expirationTime')} />
      ...
      {/* Borrower */}
      <FormItem>
        <Label label={t('marketplace.fundOpportunity.borrower')} />

        <Controller
          name={`actions.${actionIndex}.inputs.borrower`}
          control={control}
          render={({field: {onChange, value, name}}) => (
            <NumberInput {...{onChange, value, name}} placeholder="0x..." />
          )}
        />
      </FormItem>
      ...
      {/* Is persistent */}
      <FormItem>
        <Label label={t('marketplace.fundOpportunity.isPersistent')} />
      ...
      {(fundingSource as FundingSource) === 'AAVE' && (
        <>
          {/* Interest Rate Type */}
          <FormItem>
            <Label label={t('marketplace.fundOpportunity.interestRateType')} />
...
const DoubleItem = styled.div.attrs({
  className:
    'flex flex-col tablet:flex-row w-full space-y-2 tablet:space-x-2 tablet:space-y-0',
})``;
