...
export const ActiveContent = () => {
  ...
  const doughnutData = {
    labels: ['Treasury', 'Aave'],
    datasets: [
      {
        label: 'Asset allocation',
        data: [totalAssetValue - aaveNetWorth, aaveNetWorth],
        backgroundColor: ['#22d3ee', '#22d4ae'],
        hoverOffset: 4,
      },
    ],
  };
        ...
        <ActiveDataContainer>
          <ButtonsContainer>
            <ButtonText
              ...
              onClick={() =>
                navigate(generatePath(CreateGroupProposal, {network, dao: dao}))
              }
            />
            <ButtonText
              mode="primary"
          ...
          <ActiveGroupsList groupsData={groupData} />
        ...
        {groupsData.length ? (
          groupsData.map(group => (
            <ActiveGroupCardContainer key={group.id}>
              <IconPerson className="w-2 tablet:w-3 h-2 tablet:h-3 text-ui-400" />
              <ActiveGroupCardData>
...
const ActiveGroupsContainer = styled.div.attrs({
  className:
    'flex space-x-2 tablet:space-x-3 overflow-x-auto h-16 desktop:h-20 w-full',
})``;

const ActiveGroupCardContainer = styled.div.attrs({
  className:
    'flex bg-ui-50 border shadow-sm rounded-lg p-2 w-20 desktop:w-25 h-14 desktop:h-16 space-x-1',
})``;

const ActiveGroupCardData = styled.div.attrs({
  className:
    'flex flex-col items-left w-full h-full justify-between space-y-1 tablet:space-y-2',
})``;
...
const ActiveInvestmentsList = ({
  ...
  const activeInvestments = [...aaveData];
...
const TagsContainer = styled.div.attrs({
  className:
    'col-span-8 tablet:col-span-7 desktop:col-span-8 flex items-center tablet:pl-1 space-x-1 tablet:space-x-3',
})``;
