...
const Proposal: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const actionPromises: Promise<Action | undefined | Action[]>[] =
      actionsToDecode.flatMap((action: DaoAction, index) => {
        const functionParams =
          client?.decoding.findInterface(action.data) ||
          pluginClient?.decoding.findInterface(action.data);

        const functionParamsCustom = findInterfaceCustomPlugins(
          action.data,
          groupedActions ? true : false
        );

        const functionName = functionParams?.functionName
          ? functionParams.functionName
          : functionParamsCustom?.functionName;
        ...
        switch (functionName) {
          ...
          case 'borrowAndTransfer':
            return decodeCreditDelegationAction(
              action.data,
              groupedActions ? true : false,
              provider,
              network
