    ...
    <Card data-testid="cardToken" bgWhite={bgWhite}>
      <TokenDetailsWithImage>
        ...
        <TokenDetails>
          ...
          <ApyPercentage
            style={{
              color: props.changeType === 'Positive' ? '#00D16C' : '#FF3B69',
            }}
          >
            {`APY ${props.tokenAPY}%`}
          </ApyPercentage>
...
const Card = styled.div.attrs(({bgWhite}: CardProps) => ({
  className: `flex flex-row justify-between space-x-4 items-center py-1.5 tablet:py-2.5 px-2.5 tablet:px-3 overflow-hidden ${
...
const MarketProperties = styled.div.attrs({
  className:
    'ft-text-sm ml-2 pl-1 tablet:pl-0 text-right tablet:space-y-1 tablet:flex-auto tablet:overflow-hidden',
})``;
