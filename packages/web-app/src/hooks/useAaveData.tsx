...
export function useAaveData(): IuseAaveData {
  ...
  const [totalAvailableBorrow, setTotalAvailableBorrow] =
    useState<Number | null>(null);
  const [currentLiquidationThreshold, setcurrentLiquidationThreshold] =
    useState<Number | null>(null);
  ...
  const getData = async (daoAddress: string, signer: any) => {
    const pool = IPool__factory.connect(
      CONTRACT_ADDRESSES['maticmum'].aavePool,
      signer
    );
    ...
    setcurrentLiquidationThreshold(
      formatDecimals(data.currentLiquidationThreshold)
    );
    setLtv(formatDecimals(data.ltv));

    const aaveTokens = tokens.filter(token =>
      token.metadata.name.includes('Aave')
    );
    ...
    for (let i = 0; i < aaveTokens.length; i++) {
      ...
      reserveData.push({
        ...
        currentVariableBorrowRate: formatRates(
