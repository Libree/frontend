...
export const useInstalledPlugins = (
...
): IUseInstalledPlugins => {
  //TODO: add support to all networks
  const network = 'mumbai';
  const [creditDelegation, setCreditDelegation] =
    useState<InstalledPluginListItem | null>(null);
  const [subgovernance, setSubgovernace] =
    useState<InstalledPluginListItem | null>(null);
  const [uniswapV3, setUniswapV3] = useState<InstalledPluginListItem | null>(
    null
  );
  const [vault, setVault] = useState<InstalledPluginListItem | null>(null);

  const getPlugin = (
    plugins: InstalledPluginListItem[] | undefined,
    id: string
  ) => {
  ...
  const setPlugins = async (daoAddress: string) => {
    ...
    setCreditDelegation(
      getPlugin(installedPlugins, PLUGIN_IDS['maticmum'].creditDelegation) ||
        null
    );
    setSubgovernace(
      getPlugin(installedPlugins, PLUGIN_IDS['maticmum'].subgovernance) || null
    );
    setUniswapV3(
      getPlugin(installedPlugins, PLUGIN_IDS['maticmum'].uniswapV3) || null
    );
